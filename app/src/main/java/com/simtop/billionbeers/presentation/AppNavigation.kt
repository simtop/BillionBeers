...
fun AppNavigation(
    ...
    NavHost(navController = navController, startDestination = BeersList) {
        composable<BeersList> {
            BeersListScreen(
                onBeerClick = { beer ->
                    navController.navigate(BeerDetail(beer))
                },
                splitInstallManager = splitInstallManager
            )
        }

        composable<BeerDetail>(
            typeMap = mapOf(typeOf<Beer>() to serializableType<Beer>())
        ) { backStackEntry ->
            ...
            val provider = providerClass.getDeclaredConstructor()
                .newInstance() as BeerDetailProvider

            provider.BeerDetailScreen(
                beer = args.beer,
