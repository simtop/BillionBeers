...
inline infix fun <A, B, C> Either<A, B>.mapRight(f: (B) -> C): Either<A, C> =
  when (this) {
    is Either.Left -> this
...
inline infix fun <A, B, C> Either<A, C>.mapLeft(f: (A) -> B): Either<B, C> =
  when (this) {
    is Either.Left -> Either.Left(f(value))
    is Either.Right -> this
  }

inline fun <A, B, C> Either<A, B>.fold(left: (A) -> C, right: (B) -> C): C =
  when (this) {
